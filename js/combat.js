...
class CombatSystem {
    ...
    async executeCombat() {
        ...
        while (totalDamage < targetDamage && round <= maxRounds) {
            ...
            for (let i = 0; i < enhancedTroops.length; i++) {
                ...
                const damage = this.calculateTroopDamage(troop, bossMechanic);
                roundDamage += damage;
    ...
    calculateTroopDamage(troop, bossMechanic = null) {
...
function initRecruitment() {
    ...
    options.forEach((unit, index) => {
        ...
        option.innerHTML = `
            ...
            <div style="margin-top: 10px; font-weight: 600; color: ${getRarityColor(unit.rarity)};">
