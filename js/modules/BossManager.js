...
export class BossManager {
    ...
    selectBossForRank(rank) {
        ...
        // Si c'est un rang infini et qu'aucun boss n'est trouvé, créer un boss par défaut
        if (!boss && isInfiniteRank(rank)) {
            boss = {
                name: `Boss Infini ${rank}`,
                targetDamage: 1000,
                mechanic: 'Aucune mécanique spéciale'
            };
        }
    ...
    applyBossMechanicsToMultiplier(multiplier, troop) {
        const mechanic = this.gameState.currentCombat.bossMechanic;
        // Correction : ne pas appliquer le malus Titan ici, il sera appliqué globalement à la fin
    ...
    // Vérifie si une unité est désactivée par la mécanique du boss courant
    isUnitDisabledByBoss(troop) {
        const mechanic = this.gameState.currentCombat.bossMechanic;
        if (!mechanic) return false;
        // Désactivation par type
        if (mechanic.includes('corps à corps') && this.gameState.hasTroopType(troop, 'Corps à corps')) return true;
        ...
        if (mechanic.includes('ténèbres') && troop.element === 'Ténèbre') return true;
