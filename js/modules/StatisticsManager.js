...
export class StatisticsManager {
    ...
    updateCombatStatistics(troopsUsed, turnDamage) {
        ...
        // Correction : stocker le vrai total du compteur global après animation
        if (
            this.gameState.currentCombat.roundDetails &&
            this.gameState.currentCombat.roundDetails.length > 0
        ) {
            this.gameState.currentCombat.roundDetails[this.gameState.currentCombat.roundDetails.length-1].damage = this.gameState.combatManager.getLastRoundTotalDamage();
        }
    }
        ...
        const modalContent = `
            <div class="modal-content" style="max-width: 600px; max-height: 80vh; overflow-y: auto;">
                ...
                <div class="modal-body">
                    ...
                    <div class="summary-section">
                        <h4>📋 Détail des dégâts par round</h4>
                        <div style="display: flex; flex-direction: column; gap: 4px;">
                            ${(this.gameState.currentCombat.roundDetails || []).map(rd =>
                                `<div>Tour ${rd.round} : <strong>${rd.damage}</strong> dégâts</div>`
                            ).join('')}
                        </div>
                    </div>
