...
export class ConsumableManager {
    ...
    addConsumable(consumableType, gameState) {
        ...
        if (this.consumables.length >= 3) {
            gameState.showNotification('Inventaire de consommables plein (3 max) !', 'error');
            return;
        }
    ...
    useConsumable(consumableId, gameState) {
        ...
        if (success) {
           ...
           // gameState.showNotification(`${consumable.name} utilisé !`, 'success');
            ...
            gameState.showNotification('Impossible d\'utiliser ce consommable !', 'error');
            return false;
        }
    }
    ...
    upgradeSynergy(synergyName, gameState) {
        ...
        gameState.showNotification(`${synergyName} améliorée au niveau ${gameState.synergyLevels[synergyName]} !`, 'success');
    }
    ...
    transformUnitFromModal(fromUnitName, toUnitName, gameState) {
        ...
        if (transformConsumables.length === 0) {
            gameState.showNotification('Vous devez posséder un consommable de transformation pour transformer des unités !', 'error');
            return;
        }
        ...
        if (baseUnit) {
            ...
            if (gameState.transformedBaseUnits[fromUnitName] >= maxTransformations) {
                gameState.showNotification(`Vous avez déjà transformé toutes vos unités ${fromUnitName} !`, 'error');
                return;
            }
            ...
            gameState.showNotification(`Aucune unité "${fromUnitName}" trouvée !`, 'error');
            return;
        }
        ...
        if (!targetUnit) {
            gameState.showNotification(`Unité cible "${toUnitName}" non trouvée !`, 'error');
            return;
        }
        ...
        if (!baseUnit) {
            ...
            if (sourceTroopIndex !== -1) {
                ...
                gameState.showNotification(`Impossible de transformer cette unité !`, 'error');
                return;
            }
        }

        // Ajouter l'unité cible à ownedUnits au lieu de availableTroops
        gameState.ownedUnits[targetUnit.name] = (gameState.ownedUnits[targetUnit.name] || 0) + 1;
        ...
        // Relancer le tirage des 7 troupes de combat pour inclure l'unité transformée
